...
function destroyDetailImage (id) {

    var result = confirm("Bạn có chắc chắn muốn xóa ảnh ?");
    if (result) { // neu nhấn == ok , sẽ send request ajax, dc r.
        $.ajax({
            url: base_url + '/admin/imagedetail/'+id, // base_url được khai báo ở đầu page == http://webshop.local
            type: 'DELETE',
            // data: {}, // dữ liệu truyền sang nếu có
